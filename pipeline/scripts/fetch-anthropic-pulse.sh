...
render_surface() {
  ...
  if [[ "$SLUG" == "news" ]]; then
    FRONTMATTER_DESC="Digest of the most recent ~15-20 Anthropic news posts — product launches, partnerships, region openings, model releases, policy updates, business announcements. Auto-refreshed daily from anthropic.com/news. Use when the user asks: \"did X just launch?\", \"what did Anthropic announce?\", \"is model Y out yet?\", \"any partnership news?\". Skip: deep technical content (digest only has title + URL + summary; for body, WebFetch the linked URL)."
  else
    FRONTMATTER_DESC="Digest of the most recent ~15-20 Anthropic research papers/posts — alignment research, evaluations, benchmarks, interpretability, the Anthropic Institute, the Anthropic Economic Index. Auto-refreshed daily from anthropic.com/research. Use when the user asks: \"any recent Anthropic research on X?\", \"is there a paper on Y?\". Skip: production model docs (use anthropic-api etc.); deep paper content (digest only — Claude WebFetches the paper for depth)."
  fi

  {
    ...
    echo ""
    echo "> *Auto-refreshed daily.* This is a rolling digest of the"
    ...
    echo "---"
    echo ""
    echo "*Auto-updated daily from $SOURCE_URL. If a digest entry"
