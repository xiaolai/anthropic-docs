# Claude Agent SDK — TypeScript Reference (v0.3.160)

**Package**: `@anthropic-ai/claude-agent-sdk@0.3.160`
...
- [Changelog Highlights](#changelog-highlights-v0277--v03160)
...
// 'model_not_found' — since v0.3.160: replaces generic 'invalid_request' when the selected
...
**Fix** (v0.3.160+): Use the new `@anthropic-ai/claude-agent-sdk/extract` subpath export:
```typescript
...
**Legacy workaround** (pre-v0.3.160): Set `pathToClaudeCodeExecutable` explicitly to the physical
...
## Changelog Highlights (v0.2.77 → v0.3.160)

| Version | Change |
|---------|--------|
| v0.3.160 | Bug fix: PostToolUse abort signal handling — aborting during a PostToolUse hook now ends the turn with a final `result` message instead of hanging the calling process |
| v0.3.160 | Parity update with Claude Code v2.1.159; no new API surface changes |
| v0.3.160 | `SDKAssistantMessageError.error` now reports `'model_not_found'` (instead of generic `'invalid_request'`) when the model ID doesn't exist/isn't available; `api_error_status` field added to `SDKResultMessage` (HTTP status of last API error); new `@anthropic-ai/claude-agent-sdk/extract` subpath with `extractFromBunfs()` for `bun build --compile` consumers |
...
**Last verified**: 2026-06-02 | **SDK version**: 0.3.160
