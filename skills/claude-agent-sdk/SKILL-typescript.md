# Claude Agent SDK — TypeScript Reference (v0.3.158)

**Package**: `@anthropic-ai/claude-agent-sdk@0.3.158`
...
// 'model_not_found' — since v0.3.158: replaces generic 'invalid_request' when the selected
...
**Fix** (v0.3.158+): Use the new `@anthropic-ai/claude-agent-sdk/extract` subpath export:
```typescript
...
**Legacy workaround** (pre-v0.3.158): Set `pathToClaudeCodeExecutable` explicitly to the physical
...
## Changelog Highlights (v0.2.77 → v0.3.158)

| Version | Change |
|---------|--------|
| v0.3.158 | Parity update with Claude Code v2.1.158; no new API surface changes |
| v0.3.158 | `SDKAssistantMessageError.error` now reports `'model_not_found'` (instead of generic `'invalid_request'`) when the model ID doesn't exist/isn't available; `api_error_status` field added to `SDKResultMessage` (HTTP status of last API error); new `@anthropic-ai/claude-agent-sdk/extract` subpath with `extractFromBunfs()` for `bun build --compile` consumers |
...
**Last verified**: 2026-05-30 | **SDK version**: 0.3.158
