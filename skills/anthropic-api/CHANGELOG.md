...
*(pending review — see draft PR `auto/2026-05-18-pending-review`)* First full baseline audit of all six surfaces; 5 known issues added (toolRunner defaultHeaders drop, toBase64 stack overflow, Batch API parallel tool call regression on 4.6 models, toolRunner container.id propagation bug, multipart upload path stripping); checkDiffSize gate failed — changes await human review before merge.
