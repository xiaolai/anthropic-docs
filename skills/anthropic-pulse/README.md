...
This skill is **deterministically rendered** from upstream HTML — no LLM agents involved. The pipeline runs every 30 minutes (via the shared matrix workflow at `.github/workflows/pipeline.yml`):
...
- **Freshness budget = the 30-min cron, not per-query.** All users of the plugin share one refresh, paid in CI minutes, not in their conversations.
