...
description: |
  ...
  Anthropic's hosted MCP connector, MCP tunnels (connecting Claude to
  private-network MCP servers without opening inbound ports), the
  computer-use tool, the code_execution tool, bash tool, extended
  thinking, message batches, prompt caching strategy at the platform
  level, citations, deploying via Bedrock or Vertex, embeddings models,
  fast-mode behavior, context editing strategies, WIF setup, billing /
  cost reports, identity & SSO setup, the Managed Agents product, or
  self-hosted sandboxes (running Managed Agent tool execution in your
  own infrastructure).
...
|---|---|
| [`SKILL-agents-and-tools.md`](SKILL-agents-and-tools.md) | Agent Skills format spec, MCP connector, remote MCP servers, MCP tunnels (private-network MCP — Research Preview), tool use (computer use, code execution, bash, define-your-own tools), tool_choice |
