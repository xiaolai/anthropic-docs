...
description: |
  ...
  MCP servers via outbound-only tunnel), the computer-use tool, the
...
|---|---|
| [`SKILL-agents-and-tools.md`](SKILL-agents-and-tools.md) | Agent Skills format spec, MCP connector, remote MCP servers, MCP tunnels (private-network MCP via outbound-only tunnel), tool use (computer use, code execution, bash, define-your-own tools), tool_choice |
