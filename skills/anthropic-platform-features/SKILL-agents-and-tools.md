...
> **Computer use — enhanced actions (`computer_20250124`, all supported models):** In addition to the basic actions (`left_click`, `type`, `key`, `mouse_move`, `screenshot`), `computer_20250124` adds: `scroll`, `left_click_drag`, `right_click`, `middle_click`, `double_click`, `triple_click`, `left_mouse_down`, `left_mouse_up` (fine-grained click), `hold_key` (hold a key for N seconds), `wait`. To hold modifier keys (Shift/Ctrl/Alt) during a click or scroll, pass the modifier name(s) in the action's `text` field — this is separate from `hold_key`. Source: [`computer-use-tool.md`](https://platform.claude.com/docs/en/agents-and-tools/tool-use/computer-use-tool.md).
