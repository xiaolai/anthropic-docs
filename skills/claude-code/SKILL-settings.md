...
| `disableWorkflows` | boolean | Disable dynamic workflows and bundled workflow commands. Default: `false`. Equivalent to `CLAUDE_CODE_DISABLE_WORKFLOWS=1`. See `code.claude.com/docs/en/workflows.md` |
...
| `pluginSuggestionMarketplaces` | array | (Managed only) Allowlist of org marketplace names whose plugins may be suggested to users via context-aware tips (v2.1.158+) |
...
| `statusLine` | object | Custom status line command. Scripts receive `COLUMNS` and `LINES` env vars so output can be sized to the terminal width (v2.1.158+). See `code.claude.com/docs/en/statusline.md` |
