...
Claude Code's `.mcp.json` requires the top-level key `"mcpServers"` (camelCase). Some tools — including VS Code's MCP config and some server docs — use `"servers"` instead. A file with the wrong top-level key silently provides no servers. As of v2.1.152, `claude mcp list` shows a configuration error instead of silently ignoring the file.
