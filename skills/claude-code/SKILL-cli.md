...
| `--dangerously-load-development-channels` | Enable [channels](https://code.claude.com/docs/en/channels-reference.md) not on the approved allowlist, for local dev/testing. Accepts `plugin:<name>@<marketplace>` and `server:<name>` entries. Prompts for confirmation | `claude --dangerously-load-development-channels server:webhook` |
...
| `--remote-control-session-name-prefix <prefix>` | Prefix for auto-generated Remote Control session names when no explicit name is set. Defaults to hostname (e.g. `myhost-graceful-unicorn`). Same as `CLAUDE_REMOTE_CONTROL_SESSION_NAME_PREFIX` env var | `claude remote-control --remote-control-session-name-prefix dev-box` |
