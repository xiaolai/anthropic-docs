...
**PreCompact** additionally receives `trigger` and `custom_instructions`:

| Field | Notes |
|---|---|
| `trigger` | What triggered compaction: `"manual"` (user ran `/compact`) or `"auto"` (context window full) |
| `custom_instructions` | Text the user passed to `/compact`. Empty string for `auto` compaction |

PreCompact hooks can block compaction by exiting with code 2 (or returning `{"decision":"block"}`). Blocking *proactive* auto-compact skips compaction; blocking *recovery* auto-compact (already at limit) surfaces the context-limit error.

**PostCompact** additionally receives `trigger` and `compact_summary`:

| Field | Notes |
|---|---|
| `trigger` | What triggered compaction: `"manual"` or `"auto"` (same values as PreCompact) |
| `compact_summary` | The conversation summary generated by the compact operation |

PostCompact hooks cannot block or modify the compaction result; they are intended for side effects (logging, external state update).
