...
intent-match time. Refreshed daily.
...
├── .github/
│   └── workflows/
│       └── pipeline.yml       ← matrix-runs the pipeline across all skills daily
...
The CI workflow at `.github/workflows/pipeline.yml` matrix-iterates over `skills/*/` daily (GitHub free-tier coalesces under load — typical end-to-end latency is sub-hour). Most runs are no-ops because `monitor.sh` exits cheaply when upstream hasn't changed.
